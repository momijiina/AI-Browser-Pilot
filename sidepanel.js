// AI Browser Pilot - Side Panel UI
...
function formatAction(action) {
  ...
  if (action.elementIndex !== undefined) parts.push(`element: [${action.elementIndex}]`);
  if (action.text) parts.push(`text: "${action.text}"`);
  if (action.url) parts.push(`url: ${action.url}`);
  if (action.key) parts.push(`key: ${action.key}`);
  if (action.direction) parts.push(`direction: ${action.direction}`);
  if (action.summary) parts.push(`summary: ${action.summary}`);
